...
# Also load from browser/data/captcha_keys.txt
...
# Load from file
_captcha_keys_file = Path(__file__).resolve().parent / "data" / "captcha_keys.txt"
if _captcha_keys_file.exists():
    _file_keys = [ln.strip() for ln in _captcha_keys_file.read_text().splitlines() 
                  if ln.strip() and not ln.startswith("#")]
    CAPTCHA_API_KEYS_LIST.extend(_file_keys)
